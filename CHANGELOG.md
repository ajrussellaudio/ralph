...
### Added
- `lib/functions.sh`: `toml_get_array` function that parses TOML array values (multi-line and single-line) and returns one item per line to stdout (task-01)
- `test/toml_parser.bats`: bats test suite covering multi-line arrays, single-line arrays, empty arrays, missing keys, trailing commas, whitespace variations, items with spaces, and single-item arrays (task-01)
...
### Changed
- `merge.md`: squash-merge `ralph/task-*` branches into the feature branch (was `--no-ff`), producing a single commit per completed task on the feature branch (#60)
