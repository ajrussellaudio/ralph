...
### Added
- `ralph status --ticket=<KEY-N>` (JIRA backend): renders the parent ticket (key, summary, status) followed by a subtask table (key, summary, status, priority, linked open Ralph PR by `@me` whose head branch contains the subtask key, or `—`); mutex with `--label`; uses new `jira_all_subtasks` helper (combined open + closed query) and existing `jira_open_subtasks` semantics; `lib/status.sh` now sources `lib/utils.sh`; smoke tests in `test/status.bats` (#148)
- JIRA feat→main feature PR handover: new `modes/jira/feature-pr.md` that lists every Done subtask of the parent ticket (key + summary) and references the parent ticket key (no `Closes #N` syntax — JIRA tickets aren't closed by GitHub keywords); same critical "never review/approve/merge" constraint as the GitHub feature-pr mode. `lib/routing.sh` JIRA path: when no open subtasks remain (all Done), checks for an existing feat→main PR via the REST `pulls` API (cross-fork-safe) — no PR → `feature-pr`, draft PR → `feature-pr` with `FEATURE_PR_NUMBER` (promote), open ready PR → `complete`. When open subtasks exist but are all blocked, routing still falls through to the no-subtasks holding state. `test/jira-routing.bats` extended with three coverage cases (#147)
- JIRA merge cycle: new `modes/jira/merge.md` that reuses the standard merge + downstream-rebase flow without GitHub-issue close behaviour; `post_merge_cleanup` (extracted to `lib/cleanup.sh`) branches on `TASK_BACKEND` and, on the JIRA path, parses the merged PR's head branch for the project-key ticket and calls `jira_transition <KEY> "Done"`. Routing for JIRA mode reaches `merge`/`fix`/`fix-bot` via the shared review backends (copilot bot or HTML-comment sentinels) using the existing project-key branch filter. Mock `test/helpers/gh` extended with `MOCK_PR_VIEW_STATE_RESPONSE`, `MOCK_PR_VIEW_HEADREF_RESPONSE`, `MOCK_PR_VIEW_CLOSING_RESPONSE`; new `test/cleanup.bats` covers the JIRA transition and GitHub fall-through; `test/jira-routing.bats` extended with merge-cycle coverage for both review backends (#146)
- JIRA subtask Priority ordering and "is blocked by" filtering: new `lib/utils.sh` helpers `jira_filter_unblocked`, `jira_pick_next`, `jira_priority_rank`, and `jira_blockers`; `jira_open_subtasks` now also returns the priority column; `lib/routing.sh` JIRA path filters blocked subtasks and selects the highest-priority remaining one (Highest > High > Medium > Low > Lowest, ascending ticket-key tie-break). When all open subtasks are blocked, routing falls through to the no-subtasks branch. Mock `test/helpers/jira` extended to route `linkedIssues(...)` JQL queries to per-key `MOCK_JIRA_BLOCKERS_<KEY>` env vars (#145)
