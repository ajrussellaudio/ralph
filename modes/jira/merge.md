...
Do **not** add any "Closes #N" GitHub-issue close behaviour — this is a JIRA-tracked task. The PR title and body convention is unchanged from GitHub-mode merges.
...
Find all open PRs targeting `{{FEATURE_BRANCH}}` whose head branch matches the JIRA project key (`feat/<key>-…`, `fix/<key>-…`, `refactor/<key>-…`) with a PR number greater than {{PR_NUMBER}}. For each, in ascending order:
...
- Fetch and rebase the downstream branch onto new `{{FEATURE_BRANCH}}`:
  ```bash
  git fetch origin <downstream-branch>
  git rebase --onto {{FEATURE_BRANCH}} <old-tip-sha> <downstream-branch>
  ```
- If the rebase succeeds and `{{TEST_CMD}}` passes: `git push --force-with-lease origin <downstream-branch>`
...
- **If you cannot resolve a conflict confidently** (e.g. tests keep failing, or the conflict is in generated/binary files): run `git rebase --abort`, open a GitHub issue titled `⚠️ Downstream rebase conflict: <branch>` describing the conflicting files, and stop.

## Step 5 — Stop

The JIRA subtask transition to **Done** is handled automatically by the outer loop after you stop — do **not** call `jira issue move` yourself.
